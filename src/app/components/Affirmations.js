...
export default function Affirmations() {
  return (
    <section className="px-5 sm:px-8 lg:px-12 py-10">
      <div className="mx-auto max-w-5xl bg-white rounded-[32px] border border-neutral-100 shadow-sm py-16 overflow-hidden">
        ...
        <div className="relative w-full overflow-hidden flex items-center group">
          ...
          <div className="flex w-max animate-affirmation-scroll gap-6 px-3">
            {scrollData.map((item, index) => (
              <div
                key={index}
                className={`w-[220px] h-[300px] sm:w-[260px] sm:h-[340px] flex-shrink-0 rounded-[24px] flex items-center justify-center p-8 text-center transition-transform hover:scale-[1.02] cursor-default ${item.color}`}
              >
                {item.type === "text" ? (
                  <span className="text-[18px] sm:text-[20px] font-medium text-neutral-800 leading-snug">
                    {item.content}
                  </span>
                ) : (
                  <span className="text-[80px] opacity-20 grayscale">{item.content}</span>
                )}
              </div>
            ))}
          </div>
